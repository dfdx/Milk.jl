## utils.jl - a set of ML utilities for parameter initalization, performance metrics, etc.
...
function Base.iterate(itr::EachBatch, start::Int)
    n_obs = size(itr.src, ndims(itr.src))
    if start > n_obs
        return nothing
    else
        dims = [(:) for _ in 1:ndims(itr.src)-1]
        val = @view itr.src[dims..., start : min(start + itr.batch_size - 1, n_obs)]
...
function eachbatch(X::AbstractArray{T,N}; size=100) where {T,N}
